...
def calcul_MonoProcessus(f ,a ,b,n): 
    somme=0
    for i in range(0,n): 
        xi = a+ (b-a) * i / float(n)
...
def calcul_MultiProcessus(inf,sup,k,f):
    sum_local=0
...
#mono-processus
debut = time.time()
sum = calcul_MonoProcessus(fn, x1 , x2, x3)
fin= time.time()
print("Valeur estimée Pi par la méthode Mono−Processus : ", sum)
...
for k in range (nb_process) :
    tableau_pid[k]=mp.Process(target=calcul_MultiProcessus, args=(k*x2//nb_process,(k+1)*x3//nb_process,k,fn))
    tableau_pid[k].start()
...
print("Valeur estimée Pi par la méthode Multi−Processus : ", som_tab)
